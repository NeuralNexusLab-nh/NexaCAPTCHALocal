...
import { generateAlgebraProblem, renderAlgebraImage } from "./algebra-renderer.js";
...
export class VerificationStore {
    ...
    async start() {
        ...
        if (this.maintainPool) {
            for (const type of ["gravity", "algebra"]) {
                await this.ensureAtLeastOne(type);
                const timer = setInterval(() => void this.refreshPool(type), config.poolRefreshIntervalMs);
                timer.unref();
    ...
    async refreshPool(type) {
        if (this.refreshingPools.has(type))
            return;
    ...
    async generatePoolEntry(type, replaceWhenFull) {
        ...
        for (let attempt = 0; attempt < 12; attempt += 1) {
            if (type === "gravity") {
                ...
                target = path.join(directory, `alg_${algebraProblem.answerX}_${algebraProblem.answerY}_${nonce}${extension}`);
            }
        ...
        const media = await this.renderQueue.run(() => {
            if (type === "gravity")
                return this.gravityRenderer(answer);
            if (!algebraProblem)
                throw new Error("Algebra problem was not generated.");
    ...
    answerFromPoolFile(type, filename) {
        const basename = path.parse(filename).name;
        if (type === "gravity")
            return basename;
        const match = /^alg_(-?\d+)_(-?\d+)_/.exec(basename);
        if (!match)
            throw new PublicError(503, "service-unavailable", "Invalid Algebra pool entry.");
