...
export class VerificationStore {
    ...
    async reconcileGravityPool() {
        const images = await readdir(this.imageDirectory).catch(() => []);
        const audio = new Set(await readdir(this.audioDirectory).catch(() => []));
        for (const image of images.filter((name) => name.endsWith(".png"))) {
            const audioName = `${path.parse(image).name}.mp3`;
            if (!audio.has(audioName))
                await this.removeTrackedFile(path.join(this.imageDirectory, image));
            audio.delete(audioName);
        }
        for (const orphan of audio) {
            if (orphan.endsWith(".mp3"))
                await this.removeTrackedFile(path.join(this.audioDirectory, orphan));
        }
    }
    async claimAudio(audioTicket) {
    ...
    async cleanup() {
        ...
        for (const name of verificationNames) {
            ...
            if (expired || (terminal && now - record.createdAt >= config.responseLifetimeMs)) {
                ...
                if (!record.mediaConsumed)
                    this.changeMediaReference(record.mediaPath, -1);
                if (record.audioTicketHash)
                    this.audioTickets.delete(record.audioTicketHash);
                if (!record.audioConsumed && record.audioPath)
                    this.changeMediaReference(record.audioPath, -1);
